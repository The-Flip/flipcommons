...
**Scalar** macros are invisible to the generated column sweep — a `name_norm()` returning garbage changes values, not column population, so no anchor fires and nothing else would notice. Each therefore carries a data-independent smoke check in `catalog_checks.sql`, including those defined in `provenance.sql` and `data_patches.sql` so a missing one shows as a gap in one block. A **table** macro feeding public views needs none: its output _is_ the swept surface. `live()` was given one anyway and it earned nothing — breaking its liveness half fires `namesake_count_disagree`, `models_has_deleted`, `title_size_disagree` and `title_size_zero_on_live`; breaking its EXCLUDE half fires eight `anchor_dark` rows. All of those name the affected view and column, which a macro-level check cannot. This holds while every caller is a swept view, so it rests on a fact rather than on structure — check that it still holds before adding a table macro somewhere else. **This is the only coverage rule with no meta-check behind it** — `check-mutations` enforces check↔mutation both ways, but nothing enforces macro↔smoke-check, and the first macro to ship without one shipped unnoticed. Pin the behaviour the macro is _relied on_ for, not the shape it happens to have: `patch_number_of` was written against a four-digit width and silently truncated a five-digit id until its smoke check said what the parse was for.
