...
A staging view is for a table **more than one view reads**. `cabinets` IS `_staging('fc.catalog_cabinet')` and a `_stg_cabinet` would be pure indirection; the layer makes a shared read shared, it doesn't wrap every table.
