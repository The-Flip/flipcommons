...
**Source free-text earns a column; a field the catalog already models does not.** `ipdb_notes`, `ipdb_notable_features` and `opdb_features` are plain columns on `models` because they're genuine unmodeled signal — fields IPDB/OPDB carry that flipcommons never surfaces, and mining them is common analysis work, so a hand-rolled `json_extract` in every consumer is the thing to prevent. The test for any other raw `extra_data` field is whether it _shadows something the catalog already models_: if so, use the modeled form and leave the source buried — the IPDB trade name defers to `manufacturer_name`, `opdb.keywords` defers to the canonical `themes` view. The long tail (`ipdb.marketing_slogans`, `opdb.common_name`, …) stays unsurfaced until an analysis needs it; promoting one is a single line here.

The same signal works for stable keys and vocabularies: **a consumer reaching past the foundation into a raw `fc.` table for an id or a vocabulary is the promotion signal**; a second consumer confirms that it belongs in the shared surface.
...
- **The `patch_*` views specifically.** They were promoted out of a campaign-local layer in flippatch, which deleted its copy in the same breath — so unlike the rest of this list they have a known consumer today (the 0189 print-citation campaign, on `patch_entry_cites` and the `root_*` family it carries). A column dropped here surfaces as a campaign emitting nothing, in the other repo, with nothing in this one failing.
...
Adding a check? Add a line breaking what it guards — the harness **enforces** that, in both directions, so a check can't ship unproven and the spec can't rot after a rename. It also refuses to run unless the baseline is clean (otherwise a check that fires on healthy data would pass every mutation expecting it) and fails on a filter that matches nothing (otherwise a typo yields a green run that tested nothing).
