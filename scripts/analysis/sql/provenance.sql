...
-- ═══ §140 ACTORS — the one thing that can assert ════════════════════════════
...
-- ═══ §130 CLAIMS — who asserted what, and how claims compete ═════════════
...
-- ═══ §150 INGEST SOURCES AND RUNS — who and when ════════════════════════════
...
-- ═══ §160 CITATION SOURCES — external evidence ══════════════════════════════
-- shared_hosts (the declared multi-tenant hosts) is GENERATED by `manage.py
-- export_shared_hosts` (`make codegen`) from apps/citation/shared_hosts.py — a Python
-- declaration with no database row behind it, so no runtime read could reach it. It
-- loads ahead of this file (see analytics.sql): DuckDB binds a macro's table
-- references at CREATE, and _shared_cdn_host below reads the view.
