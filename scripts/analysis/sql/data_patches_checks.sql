...
-- foundation_checks.sql folds `_data_patch_checks` into `foundation_checks`, so there
...
-- every failure would report twice. Exactly the arrangement provenance_checks.sql is
-- in, for exactly the same reasons.
--
...
-- longer load into a session holding only `provenance.sql` and the layers below it. A
-- layer's checks read its own views and the ones below; this file is where the patch
-- layer's do.
--
