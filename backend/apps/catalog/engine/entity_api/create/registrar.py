"""Generic create wiring for lifecycle entities.

Lifts the shared create plumbing originally grown in :mod:`taxonomy` — rate
limiting, name / slug validation and uniqueness — into one registrar.
Entity-specific shapes (detail queryset, serialize function, response schema)
are injected as callables so the same helper wires routes for taxonomy entities
*and* the richer Theme / GameplayFeature / Series / Franchise / System schemas
without duplicating code. The create *primitives* this registrar composes
(``validate_create_input``, ``assert_public_id_available``,
``create_entity_with_claims``) live in :mod:`.persist`; the delete / restore
side lives in the sibling :mod:`..delete` package.
...
from apps.provenance.rate_limits import CREATE_RATE_LIMIT_SPEC, rate_limited
