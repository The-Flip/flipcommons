"""Example-based tests for the TikTok scheme's composite identifier.

TikTok is the composite-identity stress case: the identifier is
``<user>/video/<id>`` because the id alone cannot rebuild a watch URL. The
generic invariants live in the conformance harness; this pins the composite
grammar's shapes, the deliberate non-recognition of opaque short links, and
that the composite flows intact through child minting and the deep-link
fallback.
"""
...
class TestTikTokNormalize:
    """``normalize`` accepts the watch URL or the bare composite identifier."""

    @pytest.mark.parametrize(
        "raw",
        [
            IDENTIFIER,  # bare composite id
            f"https://www.tiktok.com/@{USER}/video/{POST}",
            f"https://tiktok.com/@{USER}/video/{POST}",
    ...
    def test_every_shape_normalizes_to_the_composite_id(self, raw):
        assert tiktok.normalize(raw) == IDENTIFIER
    ...
    def test_invalid_inputs_return_none(self, raw):
        assert tiktok.normalize(raw) is None

    def test_canonical_url_rebuilds_the_watch_page(self):
        assert (
    ...
    def test_no_time_capabilities(self):
        # TikTok URLs have no seek parameter — the scheme honestly declares
...
class TestTikTokChildMinting:
    ...
    def test_deep_link_falls_back_to_the_untouched_url(self, tiktok_root):
        ...
        # canonical link — deep_linked_url returns the url unchanged rather
        # than inventing a jump parameter.
        child = get_or_create_scheme_child(
            tiktok_root, IDENTIFIER, created_by=default_actor()
        )
