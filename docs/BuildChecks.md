...
- **Build-time env vars must be declared as `ARG`s.** Multi-stage Docker does not inherit host env vars into build stages. Railway only passes service variables as build args for `ARG`s the Dockerfile explicitly declares. Currently declared: `RAILWAY_GIT_COMMIT_SHA`, `SENTRY_AUTH_TOKEN`, `SENTRY_ORG`, `SENTRY_PROJECT`, `PUBLIC_SENTRY_DSN`, `CDN_URL`, `SITE_ORIGIN`. Adding a new build-time env var is a two-step change (declare in `Dockerfile`, set on Railway service); skipping either silently turns the consumer into a no-op.
