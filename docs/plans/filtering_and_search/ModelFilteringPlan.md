...
## ✅ DONE: <a id="pr-detail"></a>PR.DETAIL — dimension detail pages
...
| what                        | where                                                                                                                                                                                                                                                                            |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| the seven hidden dimensions | `MODEL_DIMENSION_SPECS` in `_game_rows.py` — `surfaced=False`, `facet=None`, honored from the query string; wire params on `GameFilterQuerySchema`. The sparse three entered **raw**; bucket semantics remain PR.SPARSE's. `tag` has since been surfaced — see [PR.TAG](#pr-tag) |
...
## ✅ DONE: <a id="pr-rel"></a>PR.REL — relationship vocabulary
...
## ✅ DONE: <a id="pr-sparse"></a>PR.SPARSE — sparse dimensions
...
| what               | where                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| the reserved value | `UNCLASSIFIED` in `_game_rows.py`, translated to `<field>__isnull=True` by `_sparse_q` (the null-treatment inversion is commented there); the reserved-word collision was watched only by `reserved_slug_unclassified_absent` in `model_filtering.sql` at this PR's landing (deliberately no model validation, and this PR touched no Django models or migrations) — **superseded 2026-08-01**: a DB CHECK constraint (`slug != 'unclassified'`, migration `0030_reserve_unclassified_slug`) now enforces the reservation on all three vocabulary tables; the analysis check remains as a redundant watch |
...
Decisions taken with the product owner (2026-07-31): **single-select dropdowns** (superseding the multi-select line below); **no data-model changes of any kind** — the reserved-slug guard is the analysis check, not validation (the patch-ingest path bulk-creates and never runs validators, so model validation would not have covered it) — later superseded for the reservation specifically: migration `0030_reserve_unclassified_slug` (2026-08-01) added CHECK constraints, which bulk_create cannot bypass, so the validator objection does not apply to them; **the backend never injects `unclassified` into a facet option list** — its display is entirely the frontend's; **`/api/models/` untouched** — its vocabulary converges once, at public-filtering-API publication, resolving this section's "both surfaces" hedge.
...
## ✅ DONE: <a id="pr-move"></a>PR.MOVE — route and code rename
...
**`/titles` would otherwise 404 rather than redirect.** Moving `routes/titles/+page.*` out while keeping `[slug]` and `new` leaves `/titles` as a route segment with children and no index page. The product doc calls for the redirect. **The redirect is a `routes/titles/+server.ts` GET** returning `redirect(301, '/games' + url.search)` — a `+page.server.ts` alone creates no route, an endpoint matches only its own segment (children like `/titles/<slug>` and `/titles/new` are untouched), and the query string must survive because filter state lives there and shared filter URLs are in the wild. Endpoint-only routes need no indexability classification (`allRoutes()` walks only `+page.svelte`), same as `/__health`.

`resolve()` is typed against the generated route tree, so a link that goes through it fails at compile time when the listing route moves — but that net covers only `resolve()` call sites, and the rename proved it: three listing links escaped it (the faceted shell's navigation path, derived as `/${entity_type_plural}` rather than resolved, and two raw string literals). The derived path is now covered by `listingPath()` plus a `catalogKey="title"` regression test in `FacetedCatalogListing.dom.test.ts`; the literals were converted to `resolve()`.
...
## ✅ DONE: <a id="pr-tag"></a>PR.TAG — surface the tag dimension

Reverses [PR.DETAIL](#pr-detail)'s "hide it, it isn't very useful" call for `tag` only — `display_subtype`, `technology_subgeneration` and `corporate_entity` stay hidden. Product decisions (2026-08-02): `SearchableSelect` single-select labeled "Tag", bottom of the sidebar after Series, sorted like every other facet (ignoring `Tag.display_order`), count badges, a normal single-select chip. Include-only — an untagged Model means _unrecorded_, so there is no default bucket and no exclusion, and an active tag shatters most multi-model Titles into Model cards by design.

### Outcome — where it landed (2026-08-02)

| what          | where                                                                                                                                                                                                                                                                                                                                                                        |
| ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| the registry  | `tag` in `MODEL_DIMENSION_SPECS` (`_game_rows.py`) moved out of the hidden block to sit after `edge`, gaining `facet=PathFacet("tag", "tags__slug", "tags__name")`; the `ModelDimension` Literal reordered to match (the import-time order assert pins it)                                                                                                                   |
| the fan-out   | `GameFacetOptions.tag` + `with_selected(counted["tag"], _selected(f.tag), Tag)` (`_game_facets.py`) — no new query code; `PathFacet` over an M2M is the `reward_type`/`person` shape                                                                                                                                                                                         |
| the wire      | `GameFilterOptionsSchema.tag` + the `_filter_options_payload` entry (`games.py`); facets cache version `v10` → `v11` (the no-filter payload is cached `timeout=None` and the new key is a shape change)                                                                                                                                                                      |
| the frontend  | `tag` left `ChipOnlyGameParam` (`lib/filters/games.ts`); a `single()` chip after the year block (`games-filter-chips.ts`); a `SearchableSelect` section after Series (`GameFilterSidebar.svelte`)                                                                                                                                                                            |
| behavior pins | `test_game_facets.py` — `tag` in `FACET_FILTER_FIELD`/`NARROWERS`, fixture tags (`widebody` on Beta's parent+Variant and `alpha-stern`, `prototype` on the Delta twins), and an ambient `GameFilters(tag="widebody")` state in `FILTERS`: the first single-select over a multivalued join, so `facet_exclude("tag")` dropping an M2M path from a facet base is now exercised |

Verification, live at the dev API (2026-08-02): the option set is exactly the four live tags and soft-deleted tags are absent (structurally — `Tag.soft_delete_usage_blockers` forbids deleting a tag with usage rows, and value-pruning drops usage-less tags); `?tag=remake` matches nothing. Badge == click exactly for all four: `home-use` 30, `limited-edition` 123, `prototype` 86, `widebody` 88. Model grain on its own surface (`/api/public/filter/models/`) equals the usage rows exactly: 34 / 135 / 86 / 103 — cards ≤ Models everywhere, with absorption biting hardest on `limited-edition` (123 cards over 135 Models; 95 of them are Variants) as predicted. In the browser: Tag is the last control, selecting Widebody writes `?tag=widebody` (the param names `tag`, which is the whole sidebar-binding coverage), the chip reads "Widebody" and its × clears it, and `/tags/widebody` still returns its pinned listing.
