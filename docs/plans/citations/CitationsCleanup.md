...
This plan is the execution form of [CitationWriteLayerDebt.md](CitationWriteLayerDebt.md) §4 (the `CitationSourceTypeStrategy` + `recognize_url` tidy, which rebuilds the narrower "Phase 1" of [WebCitationDomains2.md](WebCitationDomains2.md) around a model-driven spine) folding in that plan's P1.2/P1.3/P1.4. It supersedes WebCitationDomains2's Phase 1. Subdomain matching's _enablement_, its PSL public-suffix guard, the funnel rounding and the `domains:` verb are in [CitationDomainGovernance.md](CitationDomainGovernance.md); **exact** host matching already shipped via [WebCitationDomainDisablement.md](WebCitationDomainDisablement.md). Neither is in scope here.
...
1. **One name for the root/child distinction, then one strategy per source type.** The root-vs-child rule is re-spelled ~23 times; the `source_type == "web"` branch recurs in shared code (`skip_locator`, `is_abstract`, the web-flatness rule, recognition-host paths). CLAUDE.md flags exactly this ("branch on type in shared code") as the smell that should trigger a model-driven pattern. We give the distinction a name (`is_root`, `roots()`/`children()`) and each source type a `CitationSourceTypeStrategy`, so the next type (podcast, forum) is a registration, not another `if`.
...
Recognition stays **off** the type strategy — it runs before the type is known, is keyed by input kind, and is already its own registry (`EXTRACTORS`). Folding it onto the type would relapse into the link→recognition conflation. See CitationWriteLayerDebt §4 for the full argument.

## The commit sequence

Dependency-ordered. 🛑 STOP after each for review before committing. Commit messages: no ephemera.

### C1 — make `normalize_host` idempotent (standalone bug fix) — `hosts.py`
...
WebCitationDomains2 P1.1. Pure refactor, behavior-preserving. Lands before the strategy because the strategy and the D2 guard read from it.

- `CitationSourceQuerySet.roots()` / `.children()` (on a manager) — replace every `filter(parent__isnull=…)` / `filter(…__parent__isnull=…)` in app code (`api.py`, `extractors.py`, `seeding.py`, `admin.py`).
...
### C3 — `CitationSourceTypeStrategy` — `models.py` (new `strategies.py`)

CitationWriteLayerDebt §4. A plain Python `ABC`, one impl per type, one registry keyed by the existing `CitationSource.SourceType` enum (not a bare `str`), with an import-time exhaustiveness assert. **Flags only this commit** — the per-type `create_child` behavior wires in C4 with the leaves it dispatches to.

- `flat_hierarchy` (web: True), `parentless_abstract` (web/magazine: True), `child_skips_locator` (web: True) as strategy properties. `STRATEGIES.keys() == set(CitationSource.SourceType)` asserted at load.
- Re-point `skip_locator` ([models.py:255](backend/apps/citation/models.py#L255)) and `is_abstract` ([models.py:259](backend/apps/citation/models.py#L259)) at strategy reads; `ABSTRACT_PARENTLESS_SOURCE_TYPES` ([models.py:252](backend/apps/citation/models.py#L252)) dissolves into `parentless_abstract`.
...
- Keep the strategy **thin** — properties plus the one `create_child` method (added C4). Resist a god-object. Only members with a real `if source_type` today; none speculative.
- **Tests:** each flag per type; the exhaustiveness assert fires on a missing type; the D2 guard rejects a web child whose parent is itself a web child, accepts the book `root → edition → page` nesting unchanged.

🛑 STOP.

### C4 — two validated child leaves — `api.py`, `extractors.py`

WebCitationDomains2 P1.2. Converge web- and scheme-child minting onto two `full_clean`-validated leaves; wire `strategy.create_child` to dispatch to them. Closes D4 (the one real `clean()` bypass). **Two flagged behavior changes.**
...
| C3     | D2 ✅, D11/D19 🏠                     | web-flatness guard; `parentless_abstract`/`skip_locator` become strategy reads; abstract-root rejection gets a data home |
