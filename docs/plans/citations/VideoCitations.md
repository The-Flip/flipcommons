...
- **Amazon Prime Video / Apple TV — not schemes at all, but a modeling boundary.** What lives at those URLs is a **movie**: a work available through many channels (Prime, Apple TV, physical media, YouTube), so it has no canonical URL. An `amazon.com/gp/video/detail/…` link is the copy consulted — the **access URL** of [CitationInstanceUrls.md](CitationInstanceUrls.md) (Wikidata P854) — not identity. A Prime scheme would enshrine access as identity and fragment one documentary into per-platform children; the region-scoped paths (`tv.apple.com/us/…`) and the amazon.com retail collision are symptoms of that mismatch, not the root cause. The shape that fits: the movie is a parentless-citable video-typed work (the `book` precedent — then blocked by video's `parentless_abstract=True`, since relaxed), the timestamp is the **work-level locator** — the lowest common denominator across every channel, which is exactly what the "type owns locator semantics, schemes speak structured values" layering already encodes; per-copy timing drift is anchored by the instance's access URL — and the streaming URL is a declared access URL, the opaque-deliverer case the URL plan already names for `archive.org/details/<id>` ("declared, not recognized"). Precedent settled the shape question — movies do not get their own citation type; the full justification, model change and seed live in **[MovieCitations.md](MovieCitations.md)**.
...
The movie case generalizes into two holes in the domain model, recorded here for the follow-up plans ([MovieCitations.md](MovieCitations.md) for the movie work itself, [CitationSourceMisclassification.md](CitationSourceMisclassification.md) for the deliverer-host recognizer):
...
Decisions from working the movie case through. The first is now built and fully documented on its own; the rest are recorded so the follow-up plans inherit them:

- **Movies are not a new citation type — a movie is a parentless-citable `video` work.** Built and shipped; the full justification (prior art, the behavior-not-medium argument), the `is_abstract` relaxation and the pinball-movie seed live in **[MovieCitations.md](MovieCitations.md)**. In brief: the type axis encodes _behavior_ and a movie behaves exactly like a video (timestamp locator, work-level identity), so its film-ness is source-row metadata; the model change keys parentless abstractness on `identifier_key` (blank = movie, citable — the `book` rule), and `video` stays the wire value and label.
...
- **Sequencing: video releases first; [CitationInstanceUrls.md](CitationInstanceUrls.md) does not gate it.** The access URL is additive (0..1) and everything video mints meanwhile is a born-digital work where identity and access collapse — no migration, no rework. Movies split in two: the citable movie **work** (search + timestamp) depends on nothing and shipped alongside video; only **access-only deliverer hosts** (streaming-URL recognition, the fourth recognizer verb above) come after and _depend on_ instance access URLs — see [MovieCitations.md's sequencing note](MovieCitations.md#is-it-safe-to-ship-movies-before-access-urls). One courtesy owed to that future: the paste flow already holds the exact string that later becomes `access_url` (the pasted URL, `?t=` and all) and currently discards it after extracting identifier + hint — thread it through when instance URLs land.
