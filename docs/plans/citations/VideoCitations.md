...
What a genuine video tweet loses turned out to be less than it looked, because of a fact specific to X: **its URLs have no seek parameter, so the deep link never existed.** Validation exists _because_ we deep-link ("a locator that renders is allowed to be sloppy; a locator that becomes a URL parameter is not" — Prior art below); on a platform where the timestamp can never become a URL parameter, freeform text is exactly as useful as validated text. And a stored locator on a web child is already legal (the patch grammar allows it deliberately) — only the interactive stage skips. So the resolution is **skip-locator softening**, a frontend follow-up: `skip_locator` changes meaning from "the locator stage is skipped" to "the locator is _unprompted but reachable_" — the common web cite looks unchanged, and a collapsed "Add a locator" affordance on the last pre-commit step (paste and re-cite paths both) serves the video tweet's `1:35`, the podcast page's `47:10` and the long article's `§ Licensing history` alike, as freeform text. Prompted types (book, magazine, video) keep their dedicated locator stage. The heavyweight alternative — contributor-declared per-post media typing — was considered and rejected: it breaks the scheme→type pairing the `identifier_key_scheme_type` CHECK rests on, makes dedup first-writer-wins on type and taxes every X cite with a question, all to validate timestamps that deep-link nowhere. It would only be worth revisiting for a mixed-media platform _with_ seekable post URLs, and no obvious one exists (Facebook's video URLs are path-discriminated, so it would be an ordinary homogeneous video scheme).
