...
- **Find the full set of sites to flip:** `grep -rn 'SUBDOMAIN-MATCHING-DISABLED' backend/apps/citation/` — DISABLE1 left that marker at step 3 and on each inverted test. Re-point step 3, re-invert those tests to assert suffix resolution and remove the markers. **Also grep the flippatch sibling repo** (`patches/0073-gameplay-features-on-models.yaml`) for the same marker — DISABLE2 commented out 0073's four `s4.american-pinball.com` cites there; uncomment them once suffix matching resolves the subdomain again.
