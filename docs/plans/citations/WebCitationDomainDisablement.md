...
### ✅ DONE - DISABLE1: `recognize_url` matches exact host only — `extractors.py`, tests
...
- **TDD (behavior change, suffix → exact) — classify each suffix-test by what it seeds vs cites, then invert only the suffix-only ones.** A test whose cited host is itself a _directly-seeded_ root still resolves under exact matching (exact match, no competition) and must stay green — inverting it would assert a falsehood and fail against the new code. A test whose cited host is only a _subdomain of_ a seeded root no longer resolves — invert it.
  - **Invert (suffix-only — cited host not seeded, only an ancestor is):**
    ...
    - patch path — `test_url_cite_on_subdomain_nests_under_registrable_root` ([test_patch_provenance.py](backend/apps/catalog/tests/test_patch_provenance.py)) (seeds `kineticist.com`, cites `static.kineticist.com`). **Lives under `apps/catalog`, not `apps/citation`** — the `apps/citation apps/claim_ingest` green gate misses it; only the full suite (pre-commit) catches it. Flips to `pytest.raises(CitationSource.DoesNotExist)`.

    Each flips to assert the _deferred_ behavior — the subdomain is **not** recognized (raises `DoesNotExist` on the patch path, mints a new root at the raw host interactively). Confirm each fails against current suffix matching, then make the flip.
...
### ✅ DONE - DISABLE2: keep patch 0073 resolvable under exact matching — flippatch
...
### ✅ DONE - DISABLE3: verify and ship — `feat/web-citation-fixes` → `main`

- **Green gate:** `cd backend && uv run pytest apps/citation apps/claim_ingest apps/catalog -q` — `apps/claim_ingest` is mandatory: DISABLE1 changes `recognize_url`, and the patch path depends on it (`_resolve_cite_source_id` → `get_or_create_web_source` → `recognize_url`), the exact surface 0073 exercises. **`apps/catalog` is mandatory too** — `test_patch_provenance.py`'s subdomain-cite test exercises that same patch path and would otherwise slip the gate (it did, the first time; pre-commit's full-suite run caught it). Then `make mypy && make quality`.
