...
2. **`desired` is not always a pure per-claim map — but the residue is exactly two hooks, audited.** All nine reconcile resolvers were checked. Seven build a claim-local desired set (the subject's own winning claims, optionally filtered against the target table's valid PKs — a uniform existence check that belongs as an engine _built-in_, not a per-resolver hook). Two need a bespoke `desired()`, of two distinct kinds:
   ...
   So the escape hatch is small and its exact shape is known. Both non-trivial hooks are eliminated by product simplifications before the extraction ([step 0](#plan-of-record)) — the cross-entity `model_abbreviations` subtraction and the sibling-coupled media primary both move to read time — leaving every resolver claim-local: **zero** bespoke `desired()` hooks and **zero** cross-entity reads, with valid-PK filtering the only built-in.
...
0. **Make the read-time-relocatable projections claim-local (product simplifications, before the extraction).** Two projections compute at write time what is really a read-time view; relocating each to the read side makes it claim-local and deletes its bespoke handling from the engine. Both are slightly behavior-changing (not pure refactors), TDD-able, ship on their own, and must land before step 3 so the reconcile extraction sees only uniform claim-local projections. Together they take the engine to **zero** escape-hatch `desired()` hooks and **zero** cross-entity reads.

   - **(0a) Model abbreviations** — the [decision above](#decision-collapse-the-model-abbreviation-cross-entity-edge): make `resolve_all_model_abbreviations` claim-local and move the Title dedup to `_serialize_model_detail`. Removes the engine's only cross-entity read and only cascade edge. Not behavior-preserving on the storage side (it grows `ModelAbbreviation`); TDD against the staleness bug.
   - **(0b) Media primary** — make `resolve_media_attachments` store each attachment's claimed `(category, is_primary)` and pick the displayed primary at read time in the existing `all_media` / `primary_media` helper: highest claimed-primary per category, auto-promote the oldest if none. **Tiebreak by `created_at`, not claim priority** — that keeps the selection out of the row (no priority column denormalized onto `EntityMedia`) at the cost of dropping priority-ordered primary contention, an edge case worth trading for the simplicity. Removes the engine's only sibling-coupled `desired()` and one `update` driver. Product cost is near-zero: `thumbnail_url` is already nullable and the read helper already exists.

1. **Kill `resolve_model`; route MachineModel through the generic per-entity path.** Register its six relationship namespaces into the _same_ `field_name → resolver` dispatch the non-MachineModel path already uses, consulted scoped by `field_names`. **Delete the slug/opdb_id revert guard rather than relocate it:** uniqueness is a global invariant whose source of truth is the DB constraint, so let those fields fall through to `IntegrityError` → the existing 422 path that `name` already relies on (interactive), while bulk keeps `resolve_unique_conflicts` for its batch-level dedup. Resolution gets out of the uniqueness business entirely — no silent revert, no special-case — instead of carrying the guard into the projection. This deletes the `isinstance` branch, `_apply_resolution`, `_build_claims_by_model`, `_get_mm_relationship_resolvers`, the side-effecting guards and the slug/opdb_id divergence in one move, and **resolves #558** with no new abstraction. Highest leverage, medium effort. First of the refactor steps.
...
3. **Extract `reconcile`** with `{create, delete, update}` and collapse the 9 copy-pasted loops onto it. Each resolver shrinks to: build queryset → `pick_winners` → build `desired_by_subject` (the only genuinely per-shape part, ~5 lines) → `reconcile`. After step 0, **every** resolver is claim-local — no bespoke `desired()` hook remains, and valid-PK target filtering is a uniform built-in, not a hook. Deletes ~500 lines. **The typed vocabulary is born here, not in a later pass** — `Projection[K, P]`, `MemberMap[K, P]`, `RowState[P]` and `Delta[K, P]` (see [Clean-sheet shape](#clean-sheet-shape)) are `reconcile`'s signature, so the generics are what make the 9 copies collapse into one; extracting with loose types and tightening afterward would reintroduce the very drift this removes. The scalar instantiation (`Projection[str, <column value>]`) firms up in step 4.
...
## Residual complexity and product levers

The refactor above is behavior-preserving: it does not touch product functionality. But the complexity that _survives_ it is not evenly distributed, and most of it traces to a single shape — **a cross-cutting, contextual or derived concern computed at write time when it is really a read-time view.** Three instances of that shape were diagnosed; two are committed into the plan of record because the relocation genuinely reduces _overall_ system complexity (not just resolution's), reusing machinery that already exists at the destination:

- **Model abbreviations → read-time dedup** — [step 0a](#plan-of-record) (full rationale in the [decision above](#decision-collapse-the-model-abbreviation-cross-entity-edge)).
- **Media primary → read-time selection** — [step 0b](#plan-of-record). Removes the only sibling-coupled `desired()` at near-zero product cost; the read helper already exists.
- **slug/opdb_id → delete the resolve-time guard, let the DB constraint + existing 422 path own uniqueness** — folded into [step 1](#plan-of-record). The earlier "reject at write time with a new check" framing was wrong: a bespoke write-time validator would spread uniqueness across three places and get priority-ordered contention backwards. Deleting the special-case and reusing the path `name` already uses is the version that actually removes complexity.

One lever remains a **candidate, not a committed step** — the plan of record stands without it. It is a _product_ decision, listed so the product owner can weigh "is this feature worth the design weight it carries?"

| Residual smell                                                                                                    | Product feature driving it                           | Product lever (read-time move) | Design payoff                                                       | Product cost            |
| ----------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- | ------------------------------ | ------------------------------------------------------------------- | ----------------------- |
| `IMAGE_FIELDS` special-case in the scalar projection denormalizes license slug + permissiveness into `extra_data` | license-aware image filtering without joining claims | join license at read time      | removes the `IMAGE_FIELDS` branch and shrinks `extra_data` handling | a read-time join (perf) |

Not a lever — `update` is honest, not a smell: an attributed relationship (gameplay `count`, alias `display`, media `category`) is a **map**, and `{create, delete, update}` is the map algebra (see [Where clean-sheet is _not_ strictly better](#where-clean-sheet-is-not-strictly-better)). Keep it; do not chase a pure `{create, delete}` set-diff by dropping `count`/`display` or by folding the value into the member key.

Irreducible — leave alone:

- **Compound-key through-rows (`Credit` = `(person, role)`).** That is the data model, not a smell — the same person can hold two roles. It stays a `Projection[CreditAssignment, None]`.
- **Scalar-column vs through-row (two write paths).** Driven by normalization: some claimed fields are columns, some are tables. The alternatives (EAV, all-JSON) are worse.

The meta-point, and the cheapest guardrail against the _next_ such smell: it is the same discipline the [Purity invariant](#invariants-to-preserve) already states — _a projection is a function of claims; anything cross-cutting, contextual or derived is a read-time view._ Held from the start, that one rule prevents model-abbreviation, media-primary and image-license alike.
