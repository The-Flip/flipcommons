...
### <a id="POST"></a> POST — uniformity polish, ratchets, de-domaining, and the optional move

#### <a id="POST1">POST1</a> — Promote parent hierarchies to explicit through-models

REF leaves `Theme.parents`/`GameplayFeature.parents` as implicit self-referential M2Ms, resolved by the kept `_parent_projection` (synthesized from `.through`, the `from_/to_` convention). Promote each to an explicit through-model (`ThemeParent`, `GameplayFeatureParent`) and **repoint the M2M at it** — `ManyToManyField("self", through="ThemeParent", through_fields=("from_theme", "to_theme"), symmetrical=False, related_name="children")` — rather than collapsing to two bare FKs: `parents`/`children` are prefetched on the theme/gameplay-feature list endpoints (`Prefetch("parents", …)`, ~16 call sites), and only the M2M descriptor makes that traversal prefetchable. Pin the through to the existing table via `Meta.db_table` + FK fields whose columns match (`from_<model>_id`/`to_<model>_id`), with `on_delete=CASCADE` on both (the auto-through's behavior, and correct — an edge is a wholly owned child of both endpoints). The migration is then `SeparateDatabaseAndState` with **empty `database_operations`** — state-only, zero DDL, no data movement; assert the generated `database_operations == []`.

The through declares only its two FKs and a `Meta.unique_together` over them — no extra columns — so its state matches the bare physical table (`(id, from_<model>_id, to_<model>_id)` with the auto-M2M's unnamed unique index) and the migration stays zero-DDL. Two specifics preserve that property. The through carries no timestamps: a materialized through is derived state whose timeline is the claim log, not row timestamps ([DataModeling.md](../../DataModeling.md)'s "don't timestamp derived rows"). And uniqueness rides `unique_together`, **not** a named `UniqueConstraint` — a named constraint the physical table lacks would leave a state/DB name mismatch a later migration mis-targets. Then add the `SelfParentSubject(from_fk, to_fk)` variant to the core `SubjectSpec` union, declare `claim_relationship_spec` on the two new through-models (`scope=FULL_TYPE`, uniqueness via `unique_together` — the REF2 validator already accepts it), and delete `_parent_projection` + its registry entry. Parents now flow through the generic builder. (`.parents.{add,set,remove,clear}` has no production callers — only tests, which keep working because the through has no required fields beyond the two FKs, so Django needs no `through_defaults`.)
