...
- Repoint the bulk-pipeline-importing test files (in `catalog/tests/`, plus `catalog/resolve/tests/test_source_scoped_presence.py` and `provenance/tests/test_claim_license_write.py`); move the pipeline's own tests into the app. Two Step-2 artifacts need **no** repoint when the pipeline moves: `test_apply.py`'s resolve-failure test mocks `apps.provenance.resolution.resolve_entities_bulk` (provenance-rooted, so the `ingestion → claim_ingest` rename does not touch it), and `provenance/tests/test_resolution.py` is a provenance registry test that stays in provenance.
