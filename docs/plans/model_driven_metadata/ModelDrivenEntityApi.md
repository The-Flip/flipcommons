...
| `extra_data` extractors (`extract_image_*`)                        | export transforms                  | rides into D → `engine/export_api` (carry catalog's third-party-image convention, not media-storage logic) |
...
- **Media in detail/list registrars — the one non-mechanical step in C.** Unlike the rest of the phase, this is not a `git mv`: it rewrites every per-router media wiring into a single engine path — the generic registrar serializes own-media when `issubclass(model, MediaSupportedModel)` (calls `media.selectors`), and accepts an optional `thumbnail_source(entity)` hook for borrowed thumbnails, into which catalog's per-entity specs pass the domain traversal (`fetch_model_media_map`, `fetch_title_media_map`, `first_thumbnail`). It consolidates today's scattered wiring into the engine for the automatic case while keeping derived thumbnails domain-owned — a genuine refactor of where media serialization happens for every entity. The suite is a weaker oracle here than for the moves (it proves the wire output unchanged, not that the wiring is reshaped correctly), so treat this as the riskiest part of C: land it as its own commit, separate from the `git mv`s, and verify own-media and borrowed-thumbnail output per entity against pre-move (see [Verification](#verification)).
