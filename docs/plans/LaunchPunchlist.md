...
- [ ] **Status page** — even a simple "is it up?" page (Better Stack free tier, or a static page) for incidents.
- [ ] **Announcement comms** — coordinate Reddit/Pinside/social timing. Stagger so traffic doesn't all arrive in one minute.
- [ ] **On-call** — who's watching Sentry / Railway / inbox the first 48h.
- [ ] **Pre-populate seed content** — make sure `make pull-ingest` has been run and the catalog isn't empty/sparse on landing pages.
- [ ] **Trial a "soft launch"** — share with a small group (Discord, friends) 48h before broad announcement to catch issues with real traffic.
