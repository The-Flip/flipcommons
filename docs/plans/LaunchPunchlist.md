...
- [x] **HTML response Cache-Control** — `cacheControlHandle` hook (#496, `0eb5599`) stamps SSR HTML and `__data.json` based on cookies: anonymous → `public, s-maxage=60, max-age=0` (Bunny edge cache, browser revalidates), authenticated → `private, no-cache`, kiosk → `private, no-store`, any `Set-Cookie` → `private, no-store`. Caddy promotes Bunny's `X-Client-IP` to `X-Real-IP` via shared-secret so the rate limiter still sees the true client behind the new hop.
