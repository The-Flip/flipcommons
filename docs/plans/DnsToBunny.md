...
- **Was Joker actually the cause of the flaky DNS?** Never established. The two hypotheses that would have exonerated Joker were both refuted — failures hit all three Sentry checker regions, and within each region they were statistically independent — but no test ever positively identified Joker either. The migration proceeded on the independent grounds below. Whether the failure rate drops is now the deciding evidence, and it reads out after the cutover.
...
**The steering cost is now recovered.** After cutover, Google and Cloudflare both resolve the apex to the same San Jose edge that `static` gets. Connect time to it is **8.0ms**, against **156ms** for the mis-steered path — consistent with the 76.3ms/9.6ms pair measured pre-cutover from a different vantage.

**Magnitude remains unexplained.** Nothing above accounts for ~26% of checks failing; the geo-steering defect is a latency and correctness problem, not obviously an availability one. If the failure rate does not fall to roughly 1%, the `ALIAS` path was an aggravating factor and the real cause is still unidentified.
