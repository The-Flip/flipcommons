...
**Audit (2026-07-14).** No patch ≤ 0038 uses `converted_from`, `bootleg_of`, `licensed_build_of` or the three tags. However, the original **pindata seed ingest** wrote ~53 `converted_from` claims that shipped to prod, which the first audit missed by grepping only patch files. So prod DOES need a data migration for `converted_from`; `bootleg_of`/`licensed_build_of` and the bootleg/licensed-build/conversion-kit tag vocabulary remain prod-clean (born in unshipped 0017/0018/0039).
