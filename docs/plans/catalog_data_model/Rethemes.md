...
- **`subordinates = False`.** Subordination is a within-Title tiebreak in `first_model_candidates`; a re-theme is always a different Title from its donor, so it never fires. A conscious decision, not an inherited default.
...
- `CrossTitleRelation` in [titles.py](../../../backend/apps/catalog/api/titles.py) — a **manual** Literal echo (`remake_of` + the three edge types), **not** derived from `RelationshipTypeLiteral` and not snapshot-guarded, so `retheme` must be added or it fails Pydantic Literal validation on the cross-title link. Rethemes always cross titles, so this is the main path, not an edge case. Fix properly: derive it from `RelationshipTypeLiteral` + the remake FK with a guard test, so no future type has to remember.
