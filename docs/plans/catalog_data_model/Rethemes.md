...
- **License is a normal, settable axis — no block, no constraint, no `records_license` flag.** A `retheme` edge carries `license_status` exactly like a copy/conversion edge: the editor shows the selector, patches set it, and the read surfaces render it. The patch campaign sets it **case by case** — Shrek (a same-manufacturer re-theme) is `licensed`; the rest stay `unknown` and become `unlicensed` only where a curator can defend it. We never mechanically infer `unlicensed` from a different manufacturer. This captures strictly more than the old binary tag and codifies nothing automatically. See [Verification](#verification).
...
- **Donor model** (and the donor manufacturer): retrieved from free text

The donor model, the donor's manufacturer and the licensing-word flag are read from the IPDB free text in `extra_data` (`ipdb.notes` / `ipdb.notable_features`); IPDB records the donor with a fixed phrasing — _"This is a re-themed game. It used to be `<Manufacturer>`'s `<year>` '`<Donor>`'."_ An em dash (—) means empty/none.

| Retheme model               | Retheme manufacturer        | Current tag            | License words | Donor model                  | Donor manufacturer |
| --------------------------- | --------------------------- | ---------------------- | ------------- | ---------------------------- | ------------------ |
...
Structured columns (retheme model, retheme manufacturer, tag) and the raw donor-bearing note, straight from the dev SQLite database (`backend/db.sqlite3`):
...
Donor model and donor manufacturer are extracted from `ipdb_notes` with the pattern `used to be <Manufacturer>'s <year> '<Donor>'` (note that manufacturers ending in _s_, e.g. Williams, use a bare apostrophe: `Williams' 1948 'Rainbow'`; Actros names two donors; Shrek states its donor in prose — "used the existing Family Guy pinball game design").
