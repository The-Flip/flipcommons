...
#### PRE1 — Model abbreviations → read-time dedup
...
#### PRE3 — slug/opdb_id → fail loudly, not silently
...
| PRE4 | **Only if careless**                 | the new read derives license from a materialized **source** old rows lack. Hard-cutting to source-only loses license/attribution on un-re-resolved images (a regression → re-resolve becomes deploy-atomic). Falling back to the `extra_data` sidecar when the source is absent makes it stale-tolerant. **Design the fallback in.** |
