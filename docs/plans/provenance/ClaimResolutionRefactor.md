...
#### ✅ DONE: <a id="PRE2">PRE2</a> — Media primary → read-time selection
...
#### ✅ DONE: <a id="PRE3">PRE3</a> — slug/opdb_id → fail loudly, not silently
...
#### ✅ DONE: <a id="PRE4">PRE4</a> — Image license → unify the write-time computation across both paths

Lift the `IMAGE_FIELDS` license-stamp branch (`resolve_effective_license` → `permissiveness_rank`/`license_slug` sidecars) into one shared helper and call it from **both** the MachineModel `_apply_resolution` path and the generic `_entities.py` `extra_data` path. This removes the **MachineModel-only divergence** — the only thing REF1 needs — with no read-side change (the read keeps reading the frozen sidecars at zero query cost). The latent source-license **staleness bug is deliberately left in place**: it is nearly theoretical (source-level license policy is a rare admin action; re-resolve heals it), and the read-time-resolution machinery that would fix it (a prefetched license index threaded through every loop caller, an N+1 guard, an old/new-shape fallback) is interim complexity [POST3](#POST3) deletes. POST3 is the full fix — it promotes image URLs to first-class media rows and resolves license off a real FK. (Removing the unused `AttributionSchema.permissiveness_rank` wire field is a separate, independent cleanup.)

#### ✅ DONE: <a id="PRE5">PRE5</a> — remove the unused operator re-resolve surface

`resolve_machine_models` is the **second** MachineModel parallel implementation — a hand-tuned full-corpus rebuild that shares `_apply_resolution` / `_build_claims_by_model` with `resolve_model` ([\_\_init\_\_.py:141](../../../backend/apps/catalog/resolve/__init__.py#L141)). Its **only** caller is the staff "Re-resolve entities" admin page ([config/admin_views.py](../../../backend/config/admin_views.py)); it is unused in practice. Delete the admin page (view + route) and `resolve_machine_models` + `_build_claims_by_model`, migrating its test callers to the generic bulk dispatch (`resolve_entities_bulk`). This leaves REF1 only `resolve_model` to delete. First, a prerequisite: generalize generic `_resolve_bulk`'s slug-only `resolve_unique_conflicts` to **every nullable-unique claim-controlled field**, discovered by introspection. This is **broader than MM `opdb_id`** — it gives first-time bulk conflict de-confliction to 9 external-ID fields across 5 models (MachineModel `opdb_id`/`ipdb_id`/`pinside_id`, Title `opdb_id`/`fandom_page_id`, Manufacturer `opdb_manufacturer_id`/`wikidata_id`, CorporateEntity `ipdb_manufacturer_id`, Person `wikidata_id`; slug is non-null and stays on its own path). Only MM `opdb_id` was ever de-conflicted in bulk before (by `resolve_machine_models`); for every other field this closes a latent ingest bug where a collision aborts the batch with `IntegrityError` — consistent with PRE3's "bulk keeps `resolve_unique_conflicts`" design, and what lets the migrated tests (and REF1's MM bulk route) keep de-confliction. TDD against a bulk collision on an MM field and at least one non-MM field (the introspection path).

The operator capability itself (re-materialize after a source-priority/license change, which touches no claim and so nothing auto-re-resolves) is genuinely useful but was a crude HTML view; its replacement is [POST2](#POST2) — a real `manage.py resolve` command on the clean post-refactor base, which is also the global `resolve` command the [stale-state discussion](#staleness) assumes is unbuilt.

**Before deleting, lift the dependency order — it is the one thing introspection can't replace.** `resolve_machine_models` (plus the admin "all" preamble) is the only place the cross-entity resolve order is written down, and PRE5 removes both. Capture that exact sequence into [POST2](#POST2)'s spec now, so POST2 _implements a written order on the unified resolvers_ rather than reverse-engineering a deleted function. Do not orphan it as an unused code constant (dead until POST2); the vision doc is its home until then.

#### <a id="staleness"></a> Stale materialized state is a completeness concern, not a correctness gate
...
| PRE4 | No                                   | both write paths now stamp identical `__license_slug`/`__permissiveness_rank` sidecars and the read path is unchanged, so existing rows read exactly as before — no materialized state migrates. (The source-level staleness PRE4 leaves in place is pre-existing, not a deploy concern; POST3 fixes it.) |
...
REF1 is byte-identical for the claim-derived columns and through-tables, but **not** for `RecordReference` (markdown backlinks), and this is the one place it changes materialized state. The generic path syncs them (`resolve_entity`/`_resolve_bulk` → `_sync_markdown_references`); `resolve_model`/`resolve_machine_models` never did. So today MachineModel `description` backlinks materialize on **bulk** ingest (already on the generic `_resolve_bulk`) but **not** on **interactive** edits — a latent inconsistency where a `RecordReference` row is present-or-stale depending on which path last wrote. Routing the interactive path through the generic resolver makes the two agree, so REF1 incidentally fixes it. It is therefore a small, real behavior change on the interactive path, owned and TDD'd in REF1 (a new MachineModel backlink sync/clear test; existing resolve→`RecordReference` coverage is Manufacturer-only). Verified: `get_markdown_fields(MachineModel) == ["description"]`, `description` is claim-controlled, and the only other sync entry point (`save_inline_markdown_field`) has no production callers.
...
**One real snag to decide up front:** `_resolve_single` mutates the **passed-in** instance (`resolve_entity(obj)` returns `obj` mutated, and callers hold that reference), while `_resolve_bulk` loads its own rows and writes them back. Collapsing `_resolve_single` into `_resolve_bulk({obj.pk})` changes that contract. The clean resolution is a thin `resolve_entity(obj)` that resolves the one-element subject set, then `obj.refresh_from_db()` before returning — but pin this in the REF4 plan rather than discovering it mid-extraction.

**Load-bearing constraint across REF2–REF4:** write `pick_winners`, `reconcile` and the `Projection` Protocol **catalog-free from birth** — they may physically live in `catalog/resolve/` for now, but they import only provenance + core and name no concrete catalog model (they receive through-model classes as arguments). Add the import-linter contract that pins this _now_, while the code is still in catalog.
...
#### ❌ WONTFIX: <a id="POST1">POST1</a> — Scope cache invalidation

**Dropped after grounding the premise in the actual cache.** The item assumed a global flush on every edit was needlessly busting a hot cache. It isn't. `invalidate_all` (now `invalidate_response_cache`) clears only a handful of slots — the /titles and /manufacturers facet-option sidebars, the locations tree, and the per-entity-type bulk-export blobs. **Entity detail pages — the hot path — are not cached at all** (serialized live on every request), so the flush never touches what most users look at. The cached surfaces are heavyweight-but-rarely-hit (facet aggregations; a rate-limited bulk dump), and on a low-write volunteer encyclopedia the cost of over-invalidation is just "the next caller of those few endpoints rebuilds once" — already negligible. The invalidation is also correctness-neutral, so this was never a bug, only a hypothetical efficiency win.

The cache also can't host the design as written: there are **no per-entity or per-dimension keys** — the export blob is one slot per entity _type_, a full-catalog dump — so "scope to the entity and dimension a reconcile touched" would require re-architecting the export endpoint into per-entity fragments, far more than the payoff justifies. The reconcile `Delta` is real and `MediaProjection` already ships a skip-if-empty check, but generalizing even that (thread the relationship Delta up; add a scalar change-signal, since the scalar/FK path never goes through `reconcile` and produces no delta) is a minor optimization, not a correctness or UX fix.

**What shipped instead** (the discoverability fix the premise's confusion actually pointed at): a policy header on `cache.py` single-sourcing what's cached, who consumes it, what's deliberately _not_, and the three invalidation triggers; plus the `invalidate_all → invalidate_response_cache` rename so the name stops reading like an app-wide flush — the exact misread that motivated this item. Open follow-up, non-blocking: profile the facet caches; if they're cheap to rebuild, dropping them shrinks the surface more than any scoping work would. Revisit POST1 only if detail pages ever become cached (which would put a real cache on the hot path).
...
**Dependency order** (the ~15-line explicit sequence the [Ordering open question](#open-questions) keeps as control flow, not a topo-sort engine — captured here from `resolve_machine_models` + the admin "all" preamble before PRE5 deletes them). Each step is `resolve_all_entities(model)` unless noted; intra-type steps (self-referential trees, parent hierarchies, aliases) are the non-FK-derivable part that makes this worth writing down:

1. Manufacturer, CorporateEntity, Person — independent roots
2. Location, then location aliases — self-referential tree, materializes `location_path`
3. Taxonomy: TechnologyGeneration, TechnologySubgeneration, DisplayType, DisplaySubtype, Cabinet, GameFormat, RewardType, Tag, CreditRole, Franchise, Series, System
4. Theme, GameplayFeature → then their parent hierarchies and all aliases
5. CorporateEntity locations
6. Title → then title abbreviations
7. MachineModel scalars → MM relationships (credits, themes, gameplay features, reward types, tags, abbreviations) → media attachments

(`target` selectors mirror the old per-type admin buttons; `all` runs the full sequence.) If REF3's `reconcile` returns a delta, POST2 can also report per-type change counts — the one feature the admin view had that's worth keeping.

#### <a id="POST3">POST3</a> — Model third-party image URLs as first-class rows
...
#### <a id="POST4">POST4</a> — Hoist the mechanism tier into `provenance/resolution/`

The plan is at [ModelDrivenCatalogRelationshipMetadata.md](../model_driven_metadata/ModelDrivenCatalogRelationshipMetadata.md).
...
**Orthogonal to resolution — parked here because the refactor makes them cheaper, not because they belong to it.** A handful of claim-machinery types recur as bare `str` / `dict[str, str]` with no name. Each lands cheaper _after_ the refactor: REF2–REF4 collapse 25 copies into a few named primitives (`pick_winners`'s `Winners[G]`, `reconcile`'s `Projection[K, P]`, the typed dispatch key), so threading a name through touches a small, already-typed surface instead of scattered locals. Lowest priority, no behavior change; each is equally valid as a standalone GitHub issue — recorded here only so the recurring "this tightening has no home" question has an answer.

##### Name the claim `field_name` type (`FieldName`)

The claim `field_name` (a.k.a. relationship namespace) is bare `str` everywhere: `claim.field_name`, `RelationshipSchema.namespace`, `ValueKeySpec.name`, `AliasType.claim_field`, the REF1 dispatch key, and the scalar projection's `K`. Give it one named type — `NewType("FieldName", str)` for real distinctness (must wrap/unwrap at boundaries) or `type FieldName = str` for documentation only — and thread it through. Deliberately **not** part of REF1–REF4 (REF3 keeps the scalar key as bare `str`, paralleling bare `int` target-pks — see [The design](#design); naming it there would over-specify the generic `K`).
...
The cheapest guardrail against the _next_ such smell is the [Purity invariant](#invariants-to-preserve) itself — _a projection is a function of claims; anything cross-cutting, contextual or derived is a read-time view._ Held from the start, that one rule prevents model-abbreviation and media-primary; image-license is the same smell, deferred to POST3 rather than paid down in the Before phase.

## Deferred

### <a id="DEFER1">DEFER1</a> — the general engine and DB-side views
