...
**Coverage gap REF3 made concrete: the Delta exists only on the membership path.** `reconcile` returns a `Delta`, but the **scalar/FK column path never goes through `reconcile`** — it is `_resolve_bulk`'s `bulk_update`, which produces no delta. So "scope cache to what reconcile wrote" covers relationship dimensions only; scalar-column invalidation stays coarse unless REF4 makes the scalar path delta-producing (it does **not**, per the [resolved Open Question](#open-questions) — the scalar write body stays out of `reconcile`) or POST1 adds a **separate scalar change-signal** (diff old-vs-new column values during the apply loop). So **POST1 interacts with REF4** and is bigger than "thread the delta up": half the writes need their own signal. Two facts to build on: `resolve_relationship` currently discards the returned Delta (one-line change to thread it up), and `MediaProjection` already ships a mini-POST1 — it invalidates only when its delta is non-empty.
