...
`static.flipcommons.org` serves whatever SvelteKit emits through its assets path: `/_app/*` and any file under `frontend/static/` referenced via `%sveltekit.assets%` or `asset()`. The `@static_non_asset` allowlist in the [Caddyfile](../Caddyfile) is the inventory, and [`test_caddyfile_static_host.py`](../backend/tests/test_caddyfile_static_host.py) holds it to that directory. Respects origin cache headers. Does not include the query string in the cache key.
...
- **Caddy cannot see the static hostname yet.** Static-origin traffic is indistinguishable from apex traffic at the origin, so anything static-specific has to be an Edge Rule on the static zone in the Bunny dashboard for now. The `@static_non_asset` block in the [Caddyfile](../Caddyfile), which 301s every non-asset path on `static.flipcommons.org` to the apex, is dormant under this chain and only matches once the static zone pulls from Railway with Forward Host Header on. [`test_caddyfile_static_host.py`](../backend/tests/test_caddyfile_static_host.py) pins it.
- **Do not repoint static at Railway without Forward Host Header on, and not before `static.flipcommons.org` is a registered, `ACTIVE` Railway custom domain.** Railway routes by `Host` and answers an unregistered hostname with `Application not found`, so every asset would fail before reaching Caddy. With the host not forwarded, every static miss reaches `@direct_origin` as a Railway-host request without `X-Origin-Auth` and is answered with a 301 to the apex, and browsers reject scripts and fonts redirected cross-origin. Either mistake is a site-wide outage until the setting is fixed.
- **A purge of an asset path has to hit both zones, apex first.** The apex zone holds a copy of everything static has pulled, so purging static alone re-pulls the same bytes from apex. The hashed `/_app/immutable/*` assets and the fonts never need one, and `version.json` ages out of both zones within two minutes because its 60-second TTL applies at each hop. In practice this means the unfingerprinted images: they reach Bunny without `Cache-Control` and sit in two 30-day caches, so replacing one without purging both zones can take 60 days to fully age out.
...
The same applies to `static.flipcommons.org`, whose pull zone proxies every path and would otherwise serve the site as HTML: the `@static_non_asset` block 301s every non-asset path on it to the apex. It is dormant until the static zone forwards its own host; see [Static edge cache](#static-edge-cache).
