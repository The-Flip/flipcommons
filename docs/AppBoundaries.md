...
Several contracts carry a `# BASELINE` block listing pre-existing violations that predate enforcement: upward imports in `App layer tiers` (e.g. `provenance -> catalog` deferred imports, `core -> accounts` leaks), domain-logic-stranded-under-`api/` in `The api layer is a leaf` (the `soft_delete` helpers consumed by catalog's ingestion layer) and the remaining `ingestion -> catalog` imports in `Ingest system does not import the catalog domain`. Each baseline is a burn-down list: fix the underlying import and delete the line. Do not add to one. import-linter errors on an unmatched `ignore_imports` entry, so a baseline left stale after its import is gone fails the build until deleted — that error is the burn-down's forcing function.
