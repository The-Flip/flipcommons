...
A `CitationSource` is the shared work or evidence object — a book, a periodical, a web page, a database record. It is a **shared record**: when many pages cite the same book they should normally point at the same source, which is what makes deduplication and consistent presentation possible.

Sources may be **hierarchical** via a self-referential parent, when the domain needs it: work → edition, publication → issue, publisher → document, web root → child page. A slug-addressed type (periodical, document) is enforced at exactly two levels — an article is cited as its issue plus a page locator, and a document's revisions and translations are sibling children under the publisher; book hierarchy stays unconstrained. The hierarchy represents source _identity_, not locator position. Children do not inherit fields from their parent — prefill in the UI is fine, inheritance in the data is not.

A source carries a pragmatic `source_type` (book / periodical / web / video / document). Its job is to drive product behavior — search, edit fields, locator prompts, rendering — not to be a bibliography ontology, so the taxonomy stays small and grows only when a new type needs distinct behavior. Each type is a plugin spec behind one registry — see [the plugin system](#the-citation-type-plugin-system) below.

Some sources are **abstract** — a container rather than citable evidence: any source with children, any scheme-holding root (a platform like the YouTube or IPDB root — recognition always resolves to a child under it), or a parentless schemeless root of a container type: web, periodical or document (a site, a publication, a publisher — a parentless document is always the body that issued it: Williams, USPTO). Contributors cite a concrete child instead — a page under a web root, an edition under a book. A parentless book with no children is itself the work, and a parentless schemeless **video is a movie** — a work available through many channels, so it has no platform home — and both stay citable. `is_abstract` is a computed display hint (on the search API) that steers the authoring UI toward children. For web roots it is also structural: the cite paths resolve a URL to a child under the matched root (described below), so the abstract root is never the cited record.
...
A source is a scheme-holder _or_ a value-holder, never both.

The third addressing scheme, beside `identifier_key`/`identifier` and a book's `isbn`, is the authored **`slug`** on slug-addressed types (`slug_addressed` on the type spec — periodical and document, the types with no natural identifier). It is a kebab handle in the system-wide slug grammar, root-unique and sibling-unique, making `<root-slug>:<child-slug>` (`billboard:1945-09-29`, `williams:wpc-95-schematic-manual`, `uspto:us4373731`) a legal patch cite ref. Slugs are authored, never minted — a cite of an undeclared child fails, exactly like an unseeded `isbn:` — and never parsed: an issue's `year`/`month`/`day` carry its date, the slug only addresses. Root slugs are unique across all slug-addressed roots, so a ref's left segment resolves without a type. A root slug may not shadow `isbn` or a scheme key (those cite prefixes parse first), a slug-addressed child's parent must be a schemeless root (enforced in `clean()` — a slugged grandchild or a child under a scheme root would be unaddressable), and presence is CHECK-enforced: slug on exactly the slug-addressed rows, root and child alike. Children with identifiers are unique within their parent. A web child sets `skip_locator` — its URL usually pins the evidence, so the cite picker never prompts for its locator. The locator is **unprompted but reachable**, with the rare case paying the cost rather than the common one: nothing mints until save, so both interactive flows keep a revisitable surface — the edit-evidence panel and the inline-citations panel (each of which also owns its flow's quote) keep a collapsed "Add a locator" affordance for the video post's `1:35` or a long web page's section heading (the input shows only when the cite already carries a locator or the contributor opens it), and patches may store a locator on any child. A **video** child (a YouTube video under the platform root) is the opposite: its URL identifies the work but the evidence lives at a moment in it, so the locator stage prompts for an optional start time, validated against a timestamp grammar and stored canonical (`0:57`, `1:35`, `1:02:03`). On read, when the platform's URLs can seek (YouTube, Vimeo), the canonical video link deep-links to that moment (`watch?v=<id>&t=95s`) — computed server-side at serialization; on platforms with no seek parameter (TikTok) the timestamp renders beside the plain link.
...
A root's recognition host(s) live in `CitationSourceRootDomain`: a normalized host (lowercased, all leading `www.` labels stripped) that resolves to the root that owns it. Recognition is **type-blind**: any root — not just a web root — may own a recognition host and the web children it mints, so a periodical or book root with its own site (e.g. a periodical root owning `pinball-magazine.com`) recognizes URLs and nests web pages exactly like a web root does. One root may own many hosts (a rebrand's old + new domain, a `.com` + `.co.uk`, an asset subdomain). It is an owned fact, set deliberately at root creation, through seed data, or by an admin — never inferred later from edited links and never via external HTTP. Every stored recognition host must be a DNS host and must not be a bare public suffix such as `com`, `co.uk`, or `github.io`; otherwise longest-suffix matching would over-match unrelated sites. Recognition matches a pasted URL's host to the root whose recognition host is the **longest label-boundary suffix** of it, so `s4.american-pinball.com` collapses to the `american-pinball.com` root while a deliberately-seeded `twip.kineticist.com` still wins over `kineticist.com` for its own subtree.

A row may additionally carry a **`path_prefix`**, scoping recognition to one publisher's slice of a **shared multi-tenant CDN host** — GoDaddy's `img1.wsimg.com` serves every customer's files under a per-tenant `/blobby/go/<uuid>` directory, so host-only attribution there is impossible and a bare registration would claim every tenant's documents for one maker. The known shared hosts are a declared table (`apps/citation/shared_hosts.py`, the pure-configuration sibling of the deliverer table), and it cuts both ways: on a shared host a **bare** row is rejected and recognition ignores any bare row that predates the guard, while on an ordinary host a `path_prefix` is rejected — path scoping exists for shared hosts only. Prefixes are stored verbatim (case-sensitive, percent-encoding untouched, only the slash framing normalized) and match at **path-segment boundaries**, so `/blobby/go/4bd4…` covers `/blobby/go/4bd4…/downloads/x.pdf` but never `/blobby/go/4bd4…-evil/…`; querystrings never participate, and a path carrying dot-segment or backslash traversal (`/tenant/../other`) is refused outright rather than canonicalized — a CDN would resolve it to another tenant's file. Uniqueness is per `(host, path_prefix)` pair, so two makers on one shared CDN host coexist under their own prefixes, and among matching rows host specificity still dominates path specificity.
...
The authored-work create form offers the types whose roots may be created interactively (`authored_root_creation` on the type spec — book, periodical and video). A video created there is a **movie** — the parentless-citable work — and is root-only: the create endpoint 422s a video `parent_id`, because a flat-hierarchy type's children (a platform's videos, a site's pages) mint from URLs and identifiers through recognition, never by hand. A **document** root is its publisher, seeded from data patches, so the form never offers the type — a document is created as a child under its publisher root, exactly like a periodical issue, and the create endpoint refuses a parentless document so a contributor who finds no match can't mint a publisher container wearing a manual's name.
...
3. **Recognition-host match** — the hostname resolves to a root via its `CitationSourceRootDomain` (longest label-boundary suffix wins, so subdomains collapse to the owning root; a path-scoped shared-CDN row additionally requires the URL's path to sit under its tenant prefix). Lower confidence: this suggests parent _reuse_ only, so the UI pre-selects the parent but still asks for child details.
...
Two idempotent helpers, `get_or_create_external_source` (scheme + identifier) and `get_or_create_web_source` (raw URL), are the entry points ingestion uses to attach evidence. A third, `get_isbn_source`, resolves an **authored work** by its globally-unique `isbn` and deliberately never creates: a work's author, publisher, year and place in a work/edition hierarchy are editorial facts no identifier yields, so an unseeded ISBN raises rather than minting a nameless row — the ingest counterpart to the authored-work create form. It also refuses a hit that has children, since an ISBN belongs to the concrete edition, not the work grouping editions. The web helper runs the same `recognize_url`, so a URL reuses an existing **child** or nests a new child — with a `reference` link, never `homepage` — under a domain-matched root; even a URL equal to a root's own homepage nests a child, never returning the abstract root. A URL whose domain matches no seeded root **raises** rather than minting a parentless source: a root web source is an abstract container, not directly-citable evidence, so the website root must exist first.
...
Heavily reused source families are pre-seeded so autocomplete is useful from the start and contributors don't recreate the same records. Seed data is grouped by kind (books, periodicals, websites) and applied through an idempotent upsert that creates or updates sources and reconciles their links, so reseeding is safe. Seeded sources can declare additional recognition-only hosts for rebrands, alternate TLDs, asset hosts and publication subdomains. Declared hosts are verbatim facts: they are normalized and validated, but not rounded to their registrable domain. Pre-seeding is especially worthwhile for bounded pinball-specific corpora — major books, periodicals, manuals and database sites.
