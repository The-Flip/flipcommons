...
Per-type and per-platform knowledge lives behind one seam, `apps/citation/citation_types/`, so core code — models, recognition, ingest, API — never names a type or scheme. There are two plugin units with different audiences:

- A **citation type** (`CitationTypeSpec`; `book.py`, `web.py`, `video.py`, …) is first-party and rare — a new one changes locator semantics and reader UX, so it is always a product decision. The spec owns the behavior shared code would otherwise branch on: hierarchy shape, abstractness, whether children skip the locator stage, and the **locator contract** (grammar, prompt, validation, the bridge to the type's structured value).
- A **scheme** (`SchemeSpec`; one module per platform under `schemes/`) is the expected third-party unit: one platform's URL recognition — host-anchored URL shapes, a bare-id grammar, the canonical URL every shape collapses to, and optional time capabilities (a deep-link builder, seek-hint extraction). A scheme belongs to exactly one type, which is what its children mint as.

The layering rule that keeps schemes small: **the type owns locator semantics; a scheme speaks only structured values.** The video type parses and formats `1:02:03`; a video scheme is handed an identifier and plain seconds, never locator text. Both units are pure — declarative facts plus stateless functions, no model imports, no DB, no I/O — and all DB work (child minting, recognition queries, instance writes) stays in core code that consumes the specs.

One registry (`citation_types/registry.py`) aggregates both units, and everything else derives from it: the `source_type` and `identifier_key` CHECK constraints (a new scheme flows into `makemigrations`, never a hand-edit), admin choices and the per-type frontend metadata codegen. Adding a scheme is one module, one registry line, one migration and a seeding data patch — a registered scheme is **inert until its platform root is seeded**. Adding a type is a backend module, a frontend module (the UX mirror of its locator grammar), registry entries and a codegen run. A registry-parametrized conformance harness (`tests/test_scheme_conformance.py`) holds every registered scheme to the shared contract automatically, so a scheme enrolls in enforcement just by being registered.
...
The schemes live in [the plugin registry](#the-citation-type-plugin-system) keyed by `identifier_key` — the registry is the authoritative list (e.g. IPDB, OPDB and X minting web pages; YouTube, Vimeo and TikTok minting videos). Each `SchemeSpec` knows how to pull an identifier from a URL, validate a bare identifier and build the canonical URL; its children mint as the scheme's owning source type — and a scheme may only recognize URL shapes **guaranteed to be that type** (recognition is syntactic, so X's mixed-media `/status/` URLs make it a web scheme, while TikTok's `/video/` path keeps it a video one). YouTube, for example, accepts any of its URL shapes — `watch?v=`, `youtu.be/`, `/shorts/`, `/embed/`, `/live/` — and collapses them to the canonical `watch?v=<id>`, so the same video cited through different shapes resolves to one child; a pasted `?t=`/`?start=` start time surfaces as a `locator_hint` that prefills the locator stage.
