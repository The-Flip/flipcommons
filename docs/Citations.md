...
Sources may be **hierarchical** via a self-referential parent, when the domain needs it: work → edition, publication → issue, web root → child page. A magazine is enforced at exactly two levels (an article is cited as its issue plus a page locator); book hierarchy stays unconstrained. The hierarchy represents source _identity_, not locator position. Children do not inherit fields from their parent — prefill in the UI is fine, inheritance in the data is not.
