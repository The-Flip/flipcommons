<!-- @component The one renderer for a related model line — name link followed by a muted "(Manufacturer · Year)" parenthetical, shown only when it disambiguates from the subject. A known manufacturer links to its page; an unknown one (against a known subject) reads as "Unknown Manufacturer". Used by every lineage and relationship-edge section on both the mobile lists and the desktop sidebar. -->
<script lang="ts">
...
<style>
  ...
  /* Manufacturer link reads as supplementary: link-colored but at the muted size. */
  .manufacturer {
    font-size: var(--font-size-0);
  }
</style>
