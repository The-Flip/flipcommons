...
/**
 * SSR load for /themes. Page 1 is **awaited** so the rows land in the server
...
export const load: PageServerLoad = async ({ fetch, url, request }) => {
  ...
  const { data, response } = await client.GET('/api/themes/', {
    params: { query: { q, page: 1 } },
  });
  if (!data) {
    throw error(response.status || 500, 'Failed to load themes');
  }
