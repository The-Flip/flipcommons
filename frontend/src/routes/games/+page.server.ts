 ...
 * the same chunked response after the cards, and the count badges never block
 * the grid. The page renders the facet promise with `{#await}`.
 */
export const load: PageServerLoad = async ({ fetch, url, request }) => {
  ...
  // page, not degrade to an empty "0 games" success (which would also mislead
  // the create prompt on a query).
  if (!cards.data) {
    throw error(cards.response.status || 500, 'Failed to load games');
  }

  const facets = client.GET('/api/pages/games', { params: { query } });
