...
export const load: LayoutServerLoad = async ({ fetch, url, request, params }) => {
  ...
  if (!data) {
    if (response?.status === 404) throw error(404, 'Technology subgeneration not found');
    throw error(response.status || 500, 'Failed to load page');
  }

  return { profile: data, jsonLd: buildEntityJsonLd(data, technologySubgeneration, url) };
};
