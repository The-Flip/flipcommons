<script lang="ts">
  ...
  import { submitDelete } from './reward-type-delete';
  ...
  let blocked = $derived<BlockedState | null>(
    blockedReferrers.length > 0
      ? {
          kind: 'referrers',
          lead: "This reward type can't be deleted because active records still point at it:",
          referrers: blockedReferrers,
          renderReferrerHref: () => null,
          renderReferrerHint: (r: BlockingReferrer) =>
            `references this reward type via ${r.relation}`,
  ...
  let impact = $derived({
    items: ['this reward type', pluralize(preview.changeset_count, 'change set')],
    note: 'You can undo this from the toast that appears on the reward types page, or restore the record later from its edit history.',
  });
</script>

<DeletePage
  entityLabel="Reward Type"
  entityName={preview.name}
  {public_id}
  submit={submitDelete}
  cancelHref={`/reward-types/${public_id}`}
  redirectAfterDelete="/reward-types"
  editHistoryHref={`/reward-types/${public_id}/edit-history`}
  {blocked}
  {impact}
/>
