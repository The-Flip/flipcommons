...
/**
 * SSR load for /cabinets. Page 1 is **awaited** so the rows land in the server
 ...
 * page rather than a misleading empty success.
 */
export const load: PageServerLoad = async ({ fetch, url, request }) => {
  const client = createServerClient(fetch, url, request);
  const q = url.searchParams.get('q') ?? '';

  const { data, response } = await client.GET('/api/cabinets/', {
    params: { query: { q, page: 1 } },
  });
  if (!data) {
    throw error(response.status || 500, 'Failed to load cabinets');
  }
