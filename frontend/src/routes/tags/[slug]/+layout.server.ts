...
export const load: LayoutServerLoad = async (event) => {
  const { profile, q } = await loadEntityPage(event, '/api/pages/tag/{public_id}', 'Tag');
  return { profile, q, jsonLd: buildEntityJsonLd(profile, tag, event.url, detailCrumbs('tag')) };
};
